	...
	if ( have_posts() ) while ( have_posts() ) : the_post(); ?> <!-- While Loop Start Here  -->
			   		...
			   		if ( !is_array( $meta ) )
						...
						if(count($images) > 1 ){
								...
								foreach ( $images as $att ) {
									...
									$src = $src[0];
								echo "<li>";?>
					<a rel="prettyPhoto[gallery2]" href='<?php echo "{$src}"; ?>' class="" title="" >
					<?php $params = array('width' => '397', 'height' => '', 'crop' => false);?>
					<img src='<?php echo kaya_thumb($src, 397, "", "t"); ?>' /> 
					</a>	</li>
							<?php	}
							echo '</ul>';
							}else{
		...
		?>
			...
			<div class="content_box">
				...
				<div class="clear"></div>
				<?php
				edit_post_link( __( 'Edit', 'petstore' ), '<span class="edit-link">', '</span>' ); ?>
			</div>
		</div>
		<!-- End Ps -->
		<?php  // Comment Section
