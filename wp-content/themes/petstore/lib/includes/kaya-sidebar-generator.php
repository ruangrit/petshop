...
class sidebar_generator {
   ...
   public  function sidebar_generator(){
        ...
        add_action('admin_enqueue_scripts', array('sidebar_generator','admin_print_scripts'));
    ...
    static function admin_menu(){
        add_submenu_page('kaya_theme_options','Sidebar Generator', 'Sidebar Generator', 'manage_options', 'multiple_sidebars', array('sidebar_generator','admin_page'));
        ...
        <div class="wrap">
            ...
            <table class="widefat page" id="sbg_table" style="width:600px;">
                ...
                <?php
                    ...
                    ?>
                    <tr>
                        <td colspan="3"><?php _e( 'No Sidebars defined',KAYA_THEME_NAME) ?></td>
                    </tr>
                    <?php
                }
                ?>
            ...
            <div class="add_sidebar">
                <a href="javascript:void(0);" onclick="return add_sidebar_link()" title="Add a sidebar" class="button-primary"><?php _e( '+ Add New Sidebar',''); ?></a>
    ...
    */
    ...
    static function edit_form(){
        ...
        ?>
     <?php if( ( get_post_type() == 'page' ) || ( get_post_type() == 'post' ) || ( get_post_type() == 'product' ) || ( get_post_type() == 'portfolio' )){ ?>
        ...
        <div id="sbg_box" class="postbox " >
            ...
            <div class="inside">
                <div class="sbg_container">
                    ...
                    <p><?php _e( 'Please select the sidebar you would like to display on this page. <strong>Note:</strong> You must first create the sidebar under Petstore Options > Sidebar Generator.',KAYA_THEME_NAME) ?>
                    </p>
                    <ul>
                    <?php 
                    ...
                    //var_dump($wp_registered_sidebars);        
                        for($i=0;$i<1;$i++){ ?>
                            ...
                            <select name="sidebar_generator[<?php echo $i?>]" style="display: none;">
                                <option value="0"<?php if($selected_sidebar[$i] == ''){ echo " selected";} ?>><?php _e( 'WP Default Sidebar',KAYA_THEME_NAME); ?></option>
                            <?php
                            $sidebars = sidebar_generator::get_sidebars();
                            if(is_array($sidebars) && !empty($sidebars)){
                                foreach($sidebars as $sidebar){
                            ...
                            <select name="sidebar_generator_replacement[<?php echo $i?>]">
                             <option value="sidebar" <?php if($selected_sidebar_replacement[$i] == ''){ echo " selected";} ?>><?php _e( 'Sidebar',KAYA_THEME_NAME); ?></option>
                            <?php
                            $sidebar_replacements = sidebar_generator::get_sidebars();
                            if(is_array($sidebar_replacements) && !empty($sidebar_replacements)){
                                foreach($sidebar_replacements as $sidebar){
                                    if($selected_sidebar_replacement[$i] == $sidebar){
                                        echo "<option value='{$sidebar}' selected >{$sidebar}</option>\n";
                                    }else{
    ...
    */
    static function get_sidebar($name="0"){
        ...
        global $wp_query, $post_item_id;
            echo  kaya_post_item_id();
        $post = $wp_query->get_queried_object();
        $selected_sidebar = get_post_meta($post_item_id, 'sbg_selected_sidebar', true);
        $selected_sidebar_replacement = get_post_meta($post_item_id, 'sbg_selected_sidebar_replacement', true);
        $did_sidebar = false;
        //echo '<div id="sidebar" class="page_sidebar">';
       // print_r($selected_sidebar);
